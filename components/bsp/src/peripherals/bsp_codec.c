...
typedef struct {
    ...
    uint8_t dev_addr;
    char *dev_name;
    esp_err_t (*init)(audio_hal_iface_samples_t);
} codec_desc_t;
 ...
 */
static esp_err_t es7210_init_default(audio_hal_iface_samples_t sample_rate);
static esp_err_t es7243_init_default(audio_hal_iface_samples_t sample_rate);
static esp_err_t es8156_init_default(audio_hal_iface_samples_t sample_rate);
static esp_err_t es8311_init_default(audio_hal_iface_samples_t sample_rate);
    ...
    if (ESP_OK == ret_val) {
        for (size_t i = 0; i < sizeof(codec_dev_list) / sizeof(codec_dev_list[0]); i++) {
            if (codec_dev_list[i].dev & s_codec_detect_flag) {
                ret_val |= codec_dev_list[i].init(sample_rate);
